...
vector<order> orders;
map<int, multimap<int, int>> sellorders;
...
void doCase(int t){
	...
	for(int i = 0; i < N; i++){
		...
		auto lclmapit = sellorders.find(B);
		if(lclmapit == sellorders.end()){
			multimap<int, int> daymap;
			daymap.insert(pair<int, int>(Y, i));
			sellorders.insert(pair<int, multimap<int, int>>(B, daymap));
		} else {
			(lclmapit->second).insert(pair<int, int>(Y, i));
		}
